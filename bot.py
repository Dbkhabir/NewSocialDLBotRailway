...
# ==================== Config ====================
# ✅ FIX: Environment variable NAME দিতে হবে, VALUE না!
...
def run_flask():
    """Railway এর জন্য Flask server - PORT bind করতে হবে"""
...
async def button_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("v_"):
        ...
        if i < len(vs):
            dl_url = vs[i]["url"]
            qual = vs[i].get("quality", "?")
            ext = vs[i].get("extension", "mp4")
            mtype = "video"
            fsize = vs[i].get("size", 0)
            is_large = vs[i].get("is_large", False)
        ...
        if i < len(aus):
            dl_url = aus[i]["url"]
            qual = "Audio"
            ext = aus[i].get("extension", "mp3")
            mtype = "audio"
            fsize = aus[i].get("size", 0)
            is_large = aus[i].get("is_large", False)
...
def main():
    if not TELEGRAM_BOT_TOKEN:
        logger.error("❌ TELEGRAM_BOT_TOKEN not set! Set it in Railway Variables.")
        return

    if not ZYLA_API_KEY:
        logger.error("❌ ZYLA_API_KEY not set! Set it in Railway Variables.")
        return

    # Flask health check server (Railway needs PORT binding)
